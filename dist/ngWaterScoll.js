angular.module("waterScroll", []).directive("waterScroll", [
  "$timeout",
  function($timeout) {
    return {
      scope: {
      ...
      link: function(scope, ele, attrs, ctrls) {
        var scropllTop,
          ...
          scrollLoading,
          bodyContainer =
            document.getElementById(scope.scrollContainer) || document.body,
        ...
        var toLoading = function() {
          if (!scope.loadDisabled && !isDisabled) {
            if (!isLoading) {
              ...
              scrollLoading = angular.element(
                document.querySelector("#scrollLoading")
              );
        ...
        ele.append(
          "<h1 id='scrollLoading' class='text-center'>" + loadingInfo + "</h1>"
        );
